...
export const ArchivedEventsOverlay = ({ open, events, onClose, onRestore, onDelete }: Props) => (
  <AnimatePresence>
    {open && (
      <motion.div
        initial={{ x: '100%' }}
      ...
      >
        <div className="flex items-center gap-3 px-5 pt-[calc(1.25rem+env(safe-area-inset-top))] pb-3">
          ...
          <h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Eventos archivados</h1>
        </div>
        ...
        <div className="px-4 pb-10">
          {events.length === 0 ? (
            <div className="mt-16 flex flex-col items-center text-center text-zinc-400">
              ...
              <p className="text-sm">No tienes eventos archivados.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {events.map((e) => (
                ...
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white" style={{ background: EVENT_COLORS[e.color] }}>
                    <EventIcon name={e.icon} size={18} />
                  </div>
                  <p className="min-w-0 flex-1 truncate font-medium text-zinc-900 dark:text-zinc-100">{e.name}</p>
                  <button
