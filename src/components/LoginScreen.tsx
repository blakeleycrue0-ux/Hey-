import { motion } from 'framer-motion'
import { LogoMark } from './LogoMark'

interface Props {
  onLogin: () => void
}

const GoogleIcon = () => (
  <svg width="18" height="18" viewBox="0 0 48 48">
    <path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z" />
    <path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z" />
    <path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z" />
    <path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 0 1-4.087 5.571l.003-.002 6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z" />
  </svg>
)

export const LoginScreen = ({ onLogin }: Props) => (
  <div className="flex min-h-screen flex-col justify-center px-6 pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)]">
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.45, ease: 'easeOut' }}
      className="mx-auto w-full max-w-sm"
    >
      <LogoMark size={80} className="mx-auto mb-7" />
      <h1 className="text-center text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-100">
        Bienvenido a Loop
      </h1>
      <p className="mt-2 text-center text-[15px] text-zinc-500 dark:text-zinc-400">
        Construye hábitos que de verdad se quedan.
      </p>

      <button
        onClick={onLogin}
        className="mt-9 flex w-full items-center justify-center gap-3 rounded-2xl bg-white dark:bg-white/[0.06] py-3.5 text-[15px] font-semibold text-zinc-800 dark:text-zinc-100 shadow-sm"
      >
        <GoogleIcon />
        Continuar con Google
      </button>

      <p className="mt-5 text-center text-xs text-zinc-400">
        Tu cuenta y tus hábitos se guardan de forma segura.
      </p>
    </motion.div>
  </div>
)
