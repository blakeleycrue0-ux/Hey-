...
export const HabitCard = ({ habit, onToggle, onOpen }: Props) => {
  ...
  return (
    <motion.div
      layout
      className="flex items-center gap-3 rounded-2xl bg-white/70 dark:bg-white/[0.04] border border-black/5 dark:border-white/[0.06] p-3 backdrop-blur-sm shadow-sm"
    >
      <button
      ...
      >
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1 text-xs text-zinc-500 dark:text-zinc-400">
            {streak > 0 && (
              <span className="flex items-center gap-0.5 font-medium" style={{ color: palette.solid }}>
      ...
      <motion.button
        ...
        style={{
          borderColor: done ? palette.solid : 'rgba(128,128,128,0.35)',
          background: done ? `linear-gradient(135deg, ${palette.from}, ${palette.to})` : 'transparent',
        }}
      >
        {done && (
          <motion.span
